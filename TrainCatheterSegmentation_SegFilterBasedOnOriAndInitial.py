...
for seq in range(1, 10):
	...
	for ind in range(3, len(imgs_in_seq)):
		...
		ori_img_path = '/home/zsj/data/CTSAWorkspace/clipped_incremental_learning/iter2/train_dataset_for_fine_segmentation/seq' + str(seq) + '/' + str(ind) + '_data.png'
		data.append(ori_img_path)
...
modelCheckpoint = ModelCheckpoint(GENERATED_PATH + "bestTrainWeight_SegFilterBasedOnOriAndEnlargedInitial" + ".h5", monitor='loss', save_best_only=True, save_weights_only=True)
modelCheckpointTest = ModelCheckpoint(GENERATED_PATH + "bestTestWeight_SegFilterBasedOnOriAndEnlargedInitial" + ".h5", monitor='val_loss', save_best_only=True, save_weights_only=True)
...
fluoroExtraction.m_Model.save_weights(GENERATED_PATH + "lastweight_SegFilterBasedOnOriAndEnlargedInitial" + ".h5")
SavePickle(history.history, GENERATED_PATH + "history_SegFilterBasedOnOriAndEnlargedInitial" + ".pickle")