...
X[0, 0][...] = imageio.imread('/home/zsj/data/debug/final.png').astype('float32') / 255.0 
...
SaveImage("/home/zsj/data/debug/final_seg.png", (Y[0, 0]*255).astype(np.uint8))
...
for seq in range(1, 10):
		...
		for i in range(0, len(imgs_in_seq) - NB_CHANNEL + 1):
			...
			for j in range(0, NB_CHANNEL):
				X[0, j][...] = imageio.imread(path_seq + imgs_in_seq[i + NB_CHANNEL - 1 - j]).astype('float32') / 255.0
				#print(path_seq + imgs_in_seq[i + NB_CHANNEL - 1 - j])
				if os.path.exists(path_seq + imgs_in_seq[i + NB_CHANNEL - 1 - j]) == False:
					print('not exist ********************************** ')
			centerline, Y = fluoroExtraction.ExtractCenterline(X)
			save_path = path + str(seq) + '/predicted_with_attention/'
