...
for i in range(score_npy_dict.shape[0]):
	for j in range(score_npy_dict.shape[1]):
		if score_npy_dict[i, j] != -1:
			ratio_list.append((score_npy_dict[i, j], (i, j)))
ratio_list.sort(reverse = True)

NB_CHANNEL = 2
data_list = []
for item in ratio_list[:400]:
	seq, img_ind = item[1][0], item[1][1]
	train_ori_img_path = '/home/zsj/data/CTSAWorkspace/clipped_incremental_learning/iter0/seq' + str(seq) + '/' + str(img_ind) + '_image_ori.png'
	train_ori_iter0_incremental_data_path = '/home/zsj/data/CTSAWorkspace/clipped_incremental_learning/iter0/train_dataset_for_fine_segmentation/' + \
		'seq' + str(seq) + '/' + str(img_ind) + '_data_extra.png'
	train_ori_iter0_incremental_label_path = '/home/zsj/data/CTSAWorkspace/clipped_incremental_learning/iter0/train_dataset_for_fine_segmentation/' + \
...
modelCheckpoint = ModelCheckpoint(GENERATED_PATH + "bestTrainWeight_FineSegmentationBasedOnIncrement_UsingFullLabel_UsingSeperateChannel" + ".h5", monitor='loss', save_best_only=True, save_weights_only=True)
modelCheckpointTest = ModelCheckpoint(GENERATED_PATH + "bestTestWeight_FineSegmentationBasedOnIncrement_UsingFullLabel_UsingSeperateChannel" + ".h5", monitor='val_loss', save_best_only=True, save_weights_only=True)
...
fluoroExtraction.m_Model.save_weights(GENERATED_PATH + "lastweight_FineSegmentationBasedOnIncrement_UsingFullLabel_UsingSeperateChannel" + ".h5")
SavePickle(history.history, GENERATED_PATH + "history_FineSegmentationBasedOnIncrement_UsingFullLabel_UsingSeperateChannel" + ".pickle")